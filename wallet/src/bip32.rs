...
impl ExtendedPrivateKey {
    ...
    pub fn derive<Path>(seed: &[u8], path: Path, cryptography: Cryptography) -> Result<ExtendedPrivateKey, Error>
        where
            Path: IntoDerivationPath,
    {
        ...
        for child in path.into()?.as_ref() {
            sk = sk.child(*child, cryptography)?;
        }
    ...
    pub fn child(&self, child: ChildNumber, cryptography: Cryptography) -> Result<ExtendedPrivateKey, Error> {
...
mod tests {
    ...
    fn test_secp256k1_derive() {
        ...
        let ext = ExtendedPrivateKey::derive(seed.as_slice(), "m/44'/60'/0'/0/0", Cryptography::Secp256k1).unwrap();
    ...
    fn test_sm2p256v1_derive() {
        let seed = Mnemonic::from(WORDS).to_seed("Root1234");
        let ext = ExtendedPrivateKey::derive(seed.as_slice(), "m/44'/60'/0'/0/0", Cryptography::Sm2p256v1).unwrap();
    ...
    fn test_sm2p256v1_derive2() {
        ...
        let ext = ExtendedPrivateKey::derive(seed.as_slice(), "m/44'/2'/3'/4/5", Cryptography::Sm2p256v1).unwrap();
        ...
        assert_eq!(hex::encode(ext.secret_key.to_bytes_be()), excepted_sk);
        let key_pair = KeyPair::from_secret_key(ext.secret_key.to_bytes_be().as_slice(), Cryptography::Sm2p256v1);
